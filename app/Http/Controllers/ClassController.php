...
class ClassController extends Controller
{
   ...
   public function savemembers(MemberFormRequest $request, $classID){
        ...
        if ($member->count()) {
            ...
            if ($find) {
                return redirect(route('subadmin.addmembers',$classID))->with('error', 'User vừa nhập đã là thành viên của lớp');
            }
            else {
                StudentsList::create([
                'ClassID'   => $classID,
          ...
          return redirect(route('subadmin.addmembers',$classID))->with('error', 'User vừa nhập không tồn tại!');
        }
   }
